export function TimeEvent({ time, color, image, index, selected, setSelected }: { time: string, color: string, image: string, index: number, selected: boolean, setSelected: (num: number) => void }) {
    return (
    <li className={`flex flex-row items-center justify-start gap-2 rounded-xl hover:cursor-pointer hover:scale-105 transition-all duration-200 hover:bg-gray-100 origin-left p-2 ${selected ? 'bg-gray-100' : ''}`} onClick={() => setSelected(index)}>
